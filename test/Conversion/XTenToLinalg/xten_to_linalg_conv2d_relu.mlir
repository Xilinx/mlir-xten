...
// CHECK:linalg.conv_2d_relu {dilations = dense<[2, 1]> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>} ins({{.*}}, {{.*}}, {{.*}} : tensor<1x27x49x42xf32>, tensor<28x27x3x3xf32>, tensor<28xf32>) outs({{.*}} : tensor<1x28x45x40xf32>)

...
// CHECK: linalg.conv_2d_tensor_add_relu {dilations = dense<1> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>} ins({{.*}}, {{.*}}, {{.*}}, {{.*}} : tensor<1x2x130x130xf32>, tensor<1x16x128x128xf32>, tensor<16x2x3x3xf32>, tensor<16xf32>) outs({{.*}} : tensor<1x16x128x128xf32>) -> tensor<1x16x128x128xf32>
...
module attributes {torch.debug_module_name = "model"} {
  func.func @forward(%arg0: !torch.vtensor<[1,2,128,128],f32>) -> !torch.vtensor<[1,16,128,128],f32> {
    ...
    %5 = "xten.conv2d_tensoradd_relu"(%arg0, %0, %bias, %2, %2, %2, %int1, %4) : (!torch.vtensor<[1,2,128,128],f32>, !torch.vtensor<[16,2,3,3],f32>, !torch.vtensor<[16],f32>, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.int, !torch.vtensor<[1,16,128,128],f32>) -> !torch.vtensor<[1,16,128,128],f32>

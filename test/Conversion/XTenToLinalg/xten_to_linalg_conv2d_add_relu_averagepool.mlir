...
// CHECK: linalg.conv_2d_tensor_add_relu_globalaveragepool {dilations = dense<1> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>} ins({{.*}}, {{.*}}, {{.*}}, {{.*}} : tensor<1x2x130x130xf32>, tensor<16x2x3x3xf32>, tensor<16xf32>, tensor<1x16x128x128xf32>) outs({{.*}} : tensor<1x16x1x1xf32>) -> tensor<1x16x1x1xf32>

...
// CHECK:linalg.conv_2d_lrelu {dilations = dense<1> : tensor<2xi64>, layer_name = "conv2d_lrelu0", strides = dense<1> : tensor<2xi64>} ins({{.*}}, {{.*}}, {{.*}} : tensor<1x3x130x130xf32>, tensor<16x3x3x3xf32>, tensor<16xf32>, f32) outs({{.*}} : tensor<1x16x128x128xf32>)
module attributes {torch.debug_module_name = "HelloWorld"}  {
  func.func @forward(%arg0: !torch.vtensor<[1,3,128,128],f32>) -> !torch.vtensor<[1,16,128,128],f32> {

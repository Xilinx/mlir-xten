//===- conv2d_xten_affine_ordered.mlir -------------------------*- MLIR -*-===//
//
// This file is licensed under the Apache License v2.0 with LLVM Exceptions.
// See https://llvm.org/LICENSE.txt for license information.
// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception
//
// (c) Copyright 2021 Xilinx Inc.
//
//===----------------------------------------------------------------------===//

// RUN: aten-opt %s -aten-to-xten -cse | FileCheck %s

// CHECK: "xten.conv2d_lrelu_maxpool"(%arg0, %0, %1, %2, %2, %2, %int1, %float1.000000e-01, %3, %3, %4, %2, %false) : (!torch.vtensor<[1,4,52,52],f32>, !torch.vtensor<[4,4,3,3],f32>, !torch.vtensor<[4],f32>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.int, !torch.float, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.bool) -> !torch.vtensor<[?,?,?,?],unk>

module attributes {torch.debug_module_name = "CONV2D_NN"}  {
  func @forward(%arg0: !torch.vtensor<[1,4,52,52],f32>) -> !torch.vtensor<[?,?,?,?],unk> {
    %int2 = torch.constant.int 2
    %int0 = torch.constant.int 0
    %int1 = torch.constant.int 1
    %false = torch.constant.bool false
    %float1.000000e-01 = torch.constant.float 1.000000e-01
    %0 = torch.vtensor.literal(dense<""> : tensor<4x4x3x3xf32>) : !torch.vtensor<[4,4,3,3],f32>
    %1 = torch.vtensor.literal(dense<[0.0526943207, -0.00343809533, -0.0374836922, -0.102738306]> : tensor<4xf32>) : !torch.vtensor<[4],f32>
    %2 = torch.prim.ListConstruct %int1, %int1 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %3 = torch.prim.ListConstruct %int1, %int1 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %4 = torch.prim.ListConstruct %int1, %int1 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %5 = torch.aten.conv2d %arg0, %0, %1, %2, %3, %4, %int1 : !torch.vtensor<[1,4,52,52],f32>, !torch.vtensor<[4,4,3,3],f32>, !torch.vtensor<[4],f32>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.int -> !torch.vtensor<[?,?,?,?],f32>
    %6 = torch.aten.leaky_relu %5, %float1.000000e-01 : !torch.vtensor<[?,?,?,?],f32>, !torch.float -> !torch.vtensor
    %7 = torch.prim.ListConstruct %int2, %int2 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %8 = torch.prim.ListConstruct %int2, %int2 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %9 = torch.prim.ListConstruct %int0, %int0 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %10 = torch.prim.ListConstruct %int1, %int1 : (!torch.int, !torch.int) -> !torch.list<!torch.int>
    %11 = torch.aten.max_pool2d %6, %7, %8, %9, %10, %false : !torch.vtensor, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.list<!torch.int>, !torch.bool -> !torch.vtensor<[?,?,?,?],unk>
    return %11 : !torch.vtensor<[?,?,?,?],unk>
  }
}


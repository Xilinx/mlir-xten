...
// CHECK:    %[[RESIZE:.*]] = torch.operator "onnx.Resize"(%[[FROM]], %[[NONE]], %[[SCALES]], %[[NONE]]) {torch.onnx.coordinate_transformation_mode = "align_corners", torch.onnx.mode = "linear"} : (!torch.vtensor<[1,256,16,16],bf16>, !torch.none, !torch.vtensor<[4],f32>, !torch.none) -> !torch.vtensor<[1,256,32,32],bf16>
...
// CHECK:    %[[SCALES:.*]] = torch.vtensor.literal(dense<[1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00]> : tensor<4xf32>) : !torch.vtensor<[4],f32>
// CHECK:    %[[NONE:.*]] = torch.constant.none
// CHECK:    %[[RESIZE:.*]] = torch.operator "onnx.Resize"(%[[FROM]], %[[NONE]], %[[SCALES]], %[[NONE]]) {torch.onnx.coordinate_transformation_mode = "pytorch_half_pixel", torch.onnx.mode = "linear"} : (!torch.vtensor<[1,256,16,16],bf16>, !torch.none, !torch.vtensor<[4],f32>, !torch.none) -> !torch.vtensor<[1,256,32,32],bf16>
// CHECK:    %[[TO:.*]] = torch_c.to_builtin_tensor %[[RESIZE]] : !torch.vtensor<[1,256,32,32],bf16> -> tensor<1x256x32x32xbf16>
// CHECK:    return %[[TO]] : tensor<1x256x32x32xbf16>
...
// CHECK:    %[[SCALES:.*]] = torch.vtensor.literal(dense<[1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00]> : tensor<4xf32>) : !torch.vtensor<[4],f32>
// CHECK:    %[[NONE:.*]] = torch.constant.none
// CHECK:    %[[RESIZE:.*]] = torch.operator "onnx.Resize"(%[[FROM]], %[[NONE]], %[[SCALES]], %[[NONE]]) {torch.onnx.coordinate_transformation_mode = "half_pixel", torch.onnx.mode = "linear"} : (!torch.vtensor<[1,256,16,16],f32>, !torch.none, !torch.vtensor<[4],f32>, !torch.none) -> !torch.vtensor<[1,256,32,32],f32>
...
func.func @resize_illegal(%arg0: tensor<1x256x16x16xbf16>) -> tensor<1x256x32x32xbf16> {
    %1 = xten_nn.resize %arg0 {coordinate_transformation_mode = 0 : i64, mode = 0 : i64, nearest_mode = 0 : i64, scales = array<f32: 1.000000e+00, 1.000000e+00, 2.000000e+00, 2.000000e+00>} : (tensor<1x256x16x16xbf16>) -> tensor<1x256x32x32xbf16>
    return %1 : tensor<1x256x32x32xbf16>
}

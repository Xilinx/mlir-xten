...
module attributes {torch.debug_module_name = "HelloWorld"}  {
  func @forward(%arg0: tensor<1x3x128x128xf32>) -> tensor<1x16x64x64xf32> {
    ...
    %2 = linalg.conv_2d_lrelu_maxpool {dilation = dense<1> : tensor<2xi64>, layer_name = "conv2d_lrelu_maxpool0", mp_dilation = dense<1> : tensor<2xi64>, mp_kernel_size = dense<2> : tensor<2xi64>, mp_padding = dense<0> : tensor<2xi64>, mp_stride = dense<2> : tensor<2xi64>, stride = dense<1> : tensor<2xi64>} ins(%0, %cst_1, %cst_3, %cst_0 : tensor<1x3x130x130xf32>, tensor<16x3x3x3xf32>, tensor<16xf32>, f32) outs(%1 : tensor<1x16x64x64xf32>) -> tensor<1x16x64x64xf32>

...
//#include "XTenToAffine.cpp.inc"

Value typeCast(OpBuilder &builder, Value val, Type destTy) {
...
Value TensorTypeCast(OpBuilder &builder, Value val) {
  ...
  if (!refType)
    return val;
  auto tensorType = RankedTensorType::get(refType.getShape(), refType.getElementType());
  return typeCast(builder, val, tensorType);
}
...
void xilinx::xten::registerXTenToAffinePass() {
    PassRegistration<XTenToAffinePass>(
      "xten-to-affine",
      "Lower XTen dialect to Affine dialect");
}
